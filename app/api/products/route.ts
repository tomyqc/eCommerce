...
export async function GET(request: Request) {
  ...
  const limit = Math.min(15, Math.max(1, Number(searchParams.get("limit")) || 9));
  ...
  return NextResponse.json(products, { headers: { "X-Total-Count": String(total), "X-Total-Pages": String(Math.max(1, Math.ceil(total / limit))) } });
}