...
const AdminSingleOrder = () => {
  ...
  const deleteOrder = async () => {
    try {
      const productsResponse = await apiClient.delete(`/api/order-product/${order?.id}`);
      if (!productsResponse.ok && productsResponse.status !== 404) {
        throw new Error("Order products could not be deleted");
      }

      const orderResponse = await apiClient.delete(`/api/orders/${order?.id}`);
      if (!orderResponse.ok && orderResponse.status !== 204) {
        const errorData = await orderResponse.json().catch(() => ({}));
        throw new Error(errorData.error || "Order could not be deleted");
      }
  ...
  return (
    <div className="bg-transparent flex justify-start max-w-screen-2xl mx-auto xl:h-full max-xl:flex-col max-xl:gap-y-5">
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div className="border border-gray-200 p-4 max-w-xl">
          <p className="text-lg font-semibold">Payment</p>
          <p>{order.paymentMethod === "online" ? "Online payment / دفع إلكتروني" : "Payment at delivery / الدفع عند الاستلام"}</p>
          <p>Status: {order.paymentStatus === "paid" ? "Paid / مدفوع" : "Not paid / غير مدفوع"}</p>
          {order.paymentProvider && <p>Provider: {order.paymentProvider}</p>}
          ...
          {orderProducts?.map((product) => (
            <div className="flex items-center gap-x-4" key={product?.id}>
              <div className="relative">
                <Image
                  src={getProductImageUrl(product?.product?.mainImage, product?.product?.inStock, product?.product?.isNew, Boolean(product?.product?.isSold || (product?.product?.couponCode && Number(product?.product?.couponPercent) > 0)))}
              ...
              <div>
                ...
                <p>
                  {formatDZD(product?.product?.price)} * {product?.quantity} items
                </p>
              </div>
            </div>
          ))}
          <div className="flex flex-col gap-y-2 mt-10">
            <p className="text-2xl">Subtotal: {formatDZD(order?.total)}</p>
            <p className="text-2xl">Tax 20%: {formatDZD(order?.total / 5)}</p>
            <p className="text-2xl">Shipping: {formatDZD(5)}</p>
            <p className="text-3xl font-semibold">
              Total: {formatDZD(order?.total + order?.total / 5 + 5)}
            </p>
          </div>
