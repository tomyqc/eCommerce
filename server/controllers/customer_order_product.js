...
const createOrderProduct = asyncHandler(async (request, response) => {
  ...
  const orderProduct = await prisma.$transaction(async (transaction) => {
    ...
    if (stockUpdate.count === 0) {
      ...
      throw new AppError("Not enough product quantity available", 409);
    }
    ...
    const updatedProduct = await transaction.product.findUnique({ where: { id: productId } });
    ...
    return transaction.customer_order_product.create({
      data: {
        customerOrderId,
        productId,
