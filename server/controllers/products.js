...
const getAllProducts = asyncHandler(async (request, response) => {
  ...
  if(mode === "admin" || mode === "home"){
    ...
    if (dividerLocation !== -1) {
      let decodedQuery = request.url.substring(dividerLocation + 1, request.url.length);
      try {
        decodedQuery = decodeURIComponent(decodedQuery);
      } catch {
        // Continue with the original query when a value is not valid encoding.
      }
      decodedQuery = decodedQuery.replace(/\+/g, " ");

      const queryArray = decodedQuery
        .split("&");
      ...
      for (let i = 0; i < queryArray.length; i++) {
        ...
        if (queryParam.includes("filters") && filterType) {
          ...
          if (filterType === "category") {
            const encodedValue = queryParam.substring(queryParam.indexOf("=") + 1);
            try {
          ...
          if (filterType === "category" && filterValue !== null) {
            filterArray.push({
              filterType,
          ...
          const operatorEnd = queryParam.indexOf("=");
          
          if (operatorStart > 0 && operatorEnd > operatorStart) {
            const filterOperator = queryParam.substring(operatorStart, operatorEnd).replace(/\]$/u, "");
...
const createProduct = asyncHandler(async (request, response) => {
  ...
  if (existingProduct) {
    throw new AppError("A product with this slug already exists. Choose a different slug.", 409);
  }
  ...
  const product = await prisma.product.create({
    data: {
      ...
      couponPercent: Math.min(100, Math.max(0, Math.floor(Number(couponPercent ?? 0)))),
    },
  });
...
const updateProduct = asyncHandler(async (request, response) => {
  ...
  const updateData = {
    ...
    rating: rating === undefined ? existingProduct.rating : Number(rating),
  ...
  updateData.inStock = updateData.quantity > 0 ? 1 : 0;
  updateData.isNew = isNew === undefined ? existingProduct.isNew : Boolean(isNew);
  updateData.isSold = isSold === undefined ? existingProduct.isSold : Boolean(isSold);
  updateData.couponCode = couponCode === undefined
    ? existingProduct.couponCode
    : (couponCode ? String(couponCode).trim().toUpperCase() : null);
  updateData.couponPercent = couponPercent === undefined
    ? existingProduct.couponPercent
    : Math.min(100, Math.max(0, Math.floor(Number(couponPercent))));

  if (!updateData.title || !updateData.slug || !updateData.mainImage || !updateData.categoryId) {
    throw new AppError("Product title, slug, image, and category are required", 400);
  }
