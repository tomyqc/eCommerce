...
const certificatePath = path.join(__dirname, '..', 'certs', 'localhost+2.pem');
const keyPath = path.join(__dirname, '..', 'certs', 'localhost+2-key.pem');
const server = fs.existsSync(certificatePath) && fs.existsSync(keyPath)
  ? https.createServer({ cert: fs.readFileSync(certificatePath), key: fs.readFileSync(keyPath) }, app)
  : app;
